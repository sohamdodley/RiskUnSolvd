...
def load_resources():
    ...
    # Extract MODULES and QUESTIONS via safe eval of the data structures
    # (resources.txt is controlled content)
    namespace = {}
    exec(raw, namespace)
...
def init_state():
    ...
    if "video_completed" not in st.session_state:
        st.session_state.video_completed = {}  # module_id -> bool
    if "stage_unlocked" not in st.session_state:
        # module_id -> {"Semi-Pro": True/False, "Pro": ..., "Legend": ...}
    ...
    if "stage_passed" not in st.session_state:
        st.session_state.stage_passed = {}  # (module_id, stage) -> bool
...
def ensure_module_state(mid):
    ...
    if mid not in st.session_state.stage_unlocked:
        ...
        # Semi-Pro unlocks only after videos
...
def start_quiz(mid, stage):
    ...
    if len(bank) < 10:
        ...
        return
    # random.sample ensures a random subset (and random order) each attempt.
    # With bank size == 10 the set is fixed but order is randomised;
    # larger banks (recommended for production) produce different question sets.
...
def page_module():
    ...
    # Keep progression logic current
